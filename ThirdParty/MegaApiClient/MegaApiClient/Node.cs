  ...
  internal class Node : INode, INodeCrypto
  {
    ...
    public void OnDeserialized(StreamingContext ctx)
    {
      ...
      if (Type == NodeType.File || Type == NodeType.Directory)
      {
        ...
        // A node may contain keys for several overlapping shares, separated by
        // '/'. The upstream implementation assumed every entry was equivalent
        // and always used the first. Real public archives can retain an obsolete
        // first entry; it decrypts to garbage and later fails the file MAC.
        // Try the advertised keys in order and accept the first one which also
        // decrypts the node attributes successfully.
        var originalMasterKey = _masterKey;
        foreach (var serializedKey in SerializedKey.Split('/'))
        {
          ...
          try
          {
            ...
            var candidateMasterKey = originalMasterKey;
            var handle = serializedKey.Substring(0, splitPosition);
            var sharedKey = _sharedKeys?.FirstOrDefault(x => x.Id == handle);
            if (sharedKey != null)
            {
              candidateMasterKey = Crypto.DecryptKey(
                sharedKey.Key.FromBase64(),
                originalMasterKey);
            }

            var candidateFullKey = Crypto.DecryptKey(encryptedKey, candidateMasterKey);
            byte[] candidateKey;
            byte[] candidateIv = null;
            byte[] candidateMetaMac = null;

            if (Type == NodeType.File)
            {
              Crypto.GetPartsFromDecryptedKey(
                candidateFullKey,
                out candidateIv,
                out candidateMetaMac,
                out candidateKey);
            }
            else
            {
              candidateKey = candidateFullKey;
            }

            var candidateAttributes = Crypto.DecryptAttributes(
              SerializedAttributes.FromBase64(),
              candidateKey);

            // Preserve the attempted values for useful diagnostics even if none
            // of the entries can decrypt this node.
            _masterKey = candidateMasterKey;
            FullKey = candidateFullKey;
            Key = candidateKey;
            Iv = candidateIv;
            MetaMac = candidateMetaMac;
            ...
            if (candidateAttributes?.Name != null &&
                candidateAttributes.Name.StartsWith(
                  "Attribute deserialization failed:",
          ...
          {
            // Try the next key entry. If all entries fail, this node remains
            // unusable and the caller's normal verification rejects it.
          }
        }
